...
# Part 1 (default): No measurement data dropout and no miscalibration
...
# Part 2: Variances adjusted to account for calibration errors introduced in LIDAR data
...
# Part 3: Variances adjusted to account measurement data dropout for short durations
...
def measurement_update(sensor_var, p_cov_check, y_k, p_check, v_check, q_check):
    ...
    q_hat = Quaternion(euler=delta_x[6:]).quat_mult_left(q_check)
...
for k in range(1, imu_f.data.shape[0]):  # start at 1 b/c we have initial prediction from gt
    ...
    F = np.vstack([np.hstack([np.eye(3), delta_t * np.eye(3), np.zeros([3,3])]),
                   np.hstack([np.zeros([3,3]), np.eye(3), -skew_symmetric(C_ns.dot(imu_f.data[k-1])) * delta_t]),
                   np.hstack([np.zeros([3,3]), np.zeros([3,3]), np.eye(3)])])

    Q = np.eye(6)

    Q[:3,:3] = Q[:3,:3] * var_imu_f * delta_t**2

    Q[3:,3:] = Q[3:,3:] * var_imu_w * delta_t**2
